...
async def retrieve_ladder_ids_for_recent_seasons(access_token: str, season_count: int, region: str) -> List[int]:
    ...
    region_ladder_ids = []
    for season_id in range(current_season_id - season_count + 1, current_season_id + 1):

        async def retrieve_leagues(league_id: int) -> dict:
            return await asyncio.get_event_loop().run_in_executor(
                None, sc2gamedata.get_league_data, access_token, season_id, league_id, region)
