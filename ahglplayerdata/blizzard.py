...
async def retrieve_ladder_ids_for_current_season(access_token: str, region: str) -> List[int]:
    ...
    league_data_list = await asyncutil.async_map_ignore_failed(retrieve_leagues, LEAGUE_IDS)
...
async def retrieve_teams_matching_filter(
    ...
    teams = await asyncutil.async_map_ignore_failed(fetch_and_filter_teams, ladder_ids)
